...
public class MainActivity extends Activity {
    ...
    static class GameView extends View {
        ...
        private final String[] colorNames = {"Pink", "Orange", "Lemon", "Mint", "Sky", "Grape"};
        ...
        private void buildBoard() {
            ...
            for (int i = 0; i < total; i++) {
                ...
                Space s = new Space(colorIndex, null, false);
                board.add(s);
            }
        ...
        private int[] pickSpecialIndexes(int total) {
        ...
        private void buildDeck() {
            ...
            for (Space s : board) if (s.specialName != null && !s.specialName.equals("Start") && !s.specialName.equals("Castle")) {
                deck.add(new Card(-1, 0, s.specialName));
            }
        }

        void drawCard() {
            if (player >= board.size() - 1) { newGame(); return; }
        ...
        private int findNextColor(int start, int colorIndex, int count) {
            int hits = 0;
        ...
        private int indexOfSpecial(String name) {
            for (int i = 0; i < board.size(); i++) if (name.equals(board.get(i).specialName)) return i;
            return player;
        }
        ...
        private void drawPlayer(Canvas c, float w, float h) {
            ...
            PointF p = cellCenter(player, 6, cell, 8, 110, w);
        ...
        @Override public boolean onTouchEvent(android.view.MotionEvent e) {
            ...
            for (int i = 0; i < board.size(); i++) {
                ...
                if (Math.abs(x - p.x) < cell / 2 && Math.abs(y - p.y) < cell / 2) {
                    ...
                    else statusSink.set("Space " + (i + 1) + ": " + colorNames[s.colorIndex] + " path.");
                    return true;
                }
            }
            return true;
        ...
        static class Space { int colorIndex; String specialName; boolean castle, shortcut, sticky; Space(int c, String n, boolean castle) { this.colorIndex = c; this.specialName = n; this.castle = castle; } }
        static class Card { int colorIndex, count; String special; Card(int c, int count, String special) { this.colorIndex = c; this.count = count; this.special = special; } }
        static class Theme { String name, emoji, place1, place2, place3; Theme(String n, String e, String a, String b, String c) { name = n; emoji = e; place1 = a; place2 = b; place3 = c; } }
    }
}
